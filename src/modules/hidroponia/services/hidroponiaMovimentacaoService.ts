...
const isSameBercarioProfile = (
  ...
  return normalizeText(ocupacao.cultura) === normalizeText(input.cultura) && normalizeText(ocupacao.variedade) === normalizeText(input.variedade);
};
...
export const createHydroMovimentacao = async (loteId: string, data: HydroMovimentacaoFormData, userId: string) => {
  ...
  if (!Number.isFinite(quantidade) || quantidade <= 0) throw new Error('Quantidade movimentada inválida.');
  if (!Number.isFinite(perda) || perda < 0 || perda > quantidade) throw new Error('Perda inválida para esta movimentação.');
  if (!data.responsavel?.trim()) throw new Error('Informe o responsável pela movimentação para rastreabilidade.');
  if (isExitStage && !isTransfer) throw new Error('Para finalizar/cancelar, selecione a bancada de origem desta produção.');
  if (!isExitStage && !data.toEstruturaId) throw new Error('Selecione uma bancada de destino para continuar.');
  if (!isExitStage && !cultura) throw new Error('Informe a cultura/verdura da bancada de destino.');
  ...
  const estruturaDestino = data.toEstruturaId ? estruturas.find((item) => item.id === data.toEstruturaId) : null;
  if (data.toEstruturaId && !estruturaDestino) throw new Error('A bancada de destino não pertence ao layout da estufa.');

  const ocupacoesDestinoAtivas = data.toEstruturaId
    ? ocupacoesAtivasEstufa.filter((item) => item.estruturaId === data.toEstruturaId && item.status === 'ativa')
    : [];
  ...
  if (data.fromOcupacaoId) {
    ...
    if (currentFromOcupacao.loteId !== loteId) throw new Error('A ocupação de origem pertence a outra produção.');
    if (currentFromOcupacao.status !== 'ativa') throw new Error('A ocupação de origem já está encerrada.');
    ...
    if (quantidade > Number(currentFromOcupacao.quantidadeAlocada || 0)) {
      throw new Error(`Quantidade acima do saldo da origem (${Number(currentFromOcupacao.quantidadeAlocada || 0)} unidades).`);
    }
  } else {
    if (quantidade > nextSaldoDisponivel) {
      throw new Error(`Quantidade acima do saldo livre da produção (${nextSaldoDisponivel} unidades).`);
    }
    nextSaldoDisponivel = Math.max(0, nextSaldoDisponivel - quantidade);
  }

  if (estruturaDestino && movedNet > 0) {
    const capacidade = Number(estruturaDestino.capacidadePlantas || estruturaDestino.quantidadeFuros || 0);
    const ocupadoAtual = ocupacoesDestinoAtivas.reduce((sum, item) => sum + Number(item.quantidadeAlocada || 0), 0);
    if (capacidade > 0 && ocupadoAtual + movedNet > capacidade) {
      throw new Error(`Movimentação excede a capacidade da bancada (capacidade: ${capacidade}, ocupado: ${ocupadoAtual}).`);
    }
  }
  ...
  if (!isExitStage && data.toEstruturaId && movedNet > 0) {
    ...
    const ocupacaoDestino = isBercarioDestino
      ? ocupacoesDestinoAtivas.find((item) => item.loteId === loteId && isSameBercarioProfile(item, { cultura, variedade, verduraId })) || null
  ...
  await createTraceabilityEventSafely(tenantId, {
    ...
    descricao: isExitStage
      ...
      : isTransfer
      ? `Produção ${lote.codigoLote} transferida da bancada ${fromEstruturaId || '-'} para ${data.toEstruturaId || '-'} (${quantidade} plantas).`
    ...
    metadata: {
      ...
      enteAnterior: {
        nome: isTransfer ? `Bancada ${fromEstruturaId || 'desconhecida'}` : lote.origemMaterialNome || lote.nomeOperacional || lote.codigoLote,
...
export const listHydroMovimentacoesByLote = async (userId: string, loteId: string): Promise<HydroMovimentacao[]> => {
  const tenantId = assertTenantId(userId);
  const supabase = getSupabaseClient();
