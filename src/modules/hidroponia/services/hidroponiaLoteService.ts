...
const resolveSetorContext = async (tenantId: string, setorId: string): Promise<{ estufaId: string; setorId: string }> => {
  ...
  const estufaComSetor = estufas.find((estufa: Estufa) => (estufa.setores || []).some((setor) => setor.id === trimmedSetorId));
...
export const createHydroLote = async (data: HydroLoteFormData, userId: string) => {
  ...
  if (!nomeOperacional) throw new Error('Informe o nome da produção para rastreabilidade.');
  if (quantidadeInicial <= 0) throw new Error('A quantidade inicial deve ser maior que zero.');
  if (!origemMaterialNome) throw new Error('Informe a origem do material (ente anterior) para rastreabilidade.');
...
export const listHydroLotes = async (userId: string, estufaId?: string): Promise<HydroLote[]> => {
  const tenantId = assertTenantId(userId);
  const supabase = getSupabaseClient();
  let req = supabase.from('hidro_lotes').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false });
...
export const updateHydroLote = async (loteId: string, data: Partial<HydroLoteFormData>, userId: string) => {
  ...
  const nextNomeOperacional = normalizeText(data.nomeOperacional !== undefined ? data.nomeOperacional : lote.nomeOperacional);
  const nextOrigemMaterialNome = normalizeText(data.origemMaterialNome !== undefined ? data.origemMaterialNome : lote.origemMaterialNome);
  const nextOrigemMaterialDocumento = normalizeText(
    data.origemMaterialDocumento !== undefined ? data.origemMaterialDocumento : lote.origemMaterialDocumento
  );
  const nextVerduraId = normalizeText(data.verduraId !== undefined ? data.verduraId : lote.verduraId);
  const nextCulturaBase = normalizeText(data.culturaBase !== undefined ? data.culturaBase : lote.culturaBase);
  const nextVariedadeBase = normalizeText(data.variedadeBase !== undefined ? data.variedadeBase : lote.variedadeBase);
  ...
  checkField('origemMaterialDocumento', normalizeText(lote.origemMaterialDocumento), nextOrigemMaterialDocumento);
  ...
  checkField('setorId', normalizeText(lote.setorId), normalizeText(setorContext?.setorId || lote.setorId));
  ...
  const { error } = await supabase.from('hidro_lotes').update(supabasePayload).eq('id', loteId).eq('tenant_id', tenantId);
...
export const syncHydroLoteStatus = async (loteId: string, userId: string) => {
  ...
  const nextStatus = lote.status === 'cancelado' ? 'cancelado' : activeByLote > 0 || saldoDisponivel > 0 ? 'ativo' : 'concluido';
