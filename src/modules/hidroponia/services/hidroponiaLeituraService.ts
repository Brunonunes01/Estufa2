...
export const createHydroLeitura = async (data: HydroLeituraFormData, userId: string) => {
  ...
  if (motorId) {
    ...
    if (aplicarEmTodosSetoresDoMotor) {
      ...
      if (invalid) throw new Error('Existe setor selecionado que não pertence ao motor escolhido.');
  ...
  const payload: Omit<HydroLeitura, 'id'> = {
    ...
    condutividadeEletrica: typeof data.condutividadeEletrica === 'number' ? data.condutividadeEletrica : null,
  ...
  const leituraId = inserted.id as string;
