...
const TodayTasks = ({ tasks, titleColor, textColor, onOpenPlantio }: TodayTasksProps) => (
    ...
    {tasks.length === 0 ? (
      ...
      tasks.slice(0, 6).map((task) => (
        <TouchableOpacity
          ...
          onPress={() => onOpenPlantio(task.plantioId)}
        >
...
const styles = StyleSheet.create({
  ...
  row: { borderRadius: 12, borderWidth: 1, padding: 12, marginBottom: 8, flexDirection: 'row', alignItems: 'center', gap: 10 },
  content: { flex: 1 },
