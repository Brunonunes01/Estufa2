...
- Se o documento não existir, o app faz fallback para `sum()` por agregação no banco anterior.
- Isso permite:
  - manter compatibilidade imediata;
  - migrar gradualmente para atualização server-side via rotinas server-side antigas;
...
## rotinas server-side antigas (next step)
